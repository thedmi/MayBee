...
namespace MayBee
{
    ...
    public static class EnumerableExtensions
    {
        ...
        {
            var firstTwo = source.Take(2).ToList();
        ...
        {
            var firstTwoMatching = source.Where(predicate).Take(2).ToList();
            return SingleAsMaybeCandidateToResult(firstTwoMatching);
        }

        private static Maybe<T> SingleAsMaybeCandidateToResult<T>(IReadOnlyList<T> firstTwo)
        {
            if (firstTwo.Count > 2)
            {
                throw new ArgumentException("List of size 0 - 2 expected", "firstTwo");
            }
